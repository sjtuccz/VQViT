...
from timm.imagenet_a_dict import imagenet_a_map2_1k, apply_a_mask_on_1k_output, imagenet_r_map2_1k, apply_r_mask_on_1k_output
    ...
    with torch.no_grad():
        ...
        for batch_idx, (input, target) in enumerate(loader):
            if args.dataset =='imagenet-a':
                target = imagenet_a_map2_1k(target).to(device)
            elif args.dataset =='imagenet-r':
                target = imagenet_r_map2_1k(target).to(device)
            ...
            if args.dataset =='imagenet-a':
                output=apply_a_mask_on_1k_output(output.cpu()).to(device)
            if args.dataset =='imagenet-r':
                output=apply_r_mask_on_1k_output(output.cpu()).to(device)
                # print(output.shape) # add mask on output
...
def main():
    ...
    elif 'imagenet-a' in args.dataset:
        args.data_dir = '../../imagenet-a/' if not args.data_dir else args.data_dir
        args.num_classes = 1000

    elif 'imagenet-r' in args.dataset:
        args.data_dir = '../../imagenet_r/OpenDataLab___ImageNet-R/raw/imagenet-r/' if not args.data_dir else args.data_dir
        args.num_classes = 1000

    elif 'imagenet-sk' in args.dataset: #imagenet-sketch
        args.data_dir = '../../imagenet_sketch/OpenDataLab___ImageNet-Sketch/raw/sketch/' if not args.data_dir else args.data_dir
        args.num_classes = 1000
