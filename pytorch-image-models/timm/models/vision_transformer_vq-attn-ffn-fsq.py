...
class vqAttention(nn.Module):
    ...
    def __init__(
        ...
        print(f'using vq in atten: {dic_n, dic_dim}')
        self.is_pre_cal = False

    def reparameterize(self):
        print('using Attention reparameterize')
        self.is_pre_cal = True
        self.q_dict = nn.Embedding(self.vq.codebook_size, self.dim)
        self.k_dict = nn.Embedding(self.vq.codebook_size, self.dim)
        self.v_dict = nn.Embedding(self.vq.codebook_size, self.dim)
        vq_dict = self.vq.reparameterize()
        vq_dict = vq_dict.unsqueeze(0)
        B, N, C = vq_dict.shape
        qkv = self.qkv(vq_dict).reshape(B, N, 3, self.num_heads, self.head_dim).permute(2, 0, 3, 1, 4)
        q, k, v = qkv.unbind(0)
        q, k = self.q_norm(q), self.k_norm(k)
        q = q * self.scale
...
class vqBlock(nn.Module):
    ...
    def __init__(
        ...
        self.attn = vqAttention(
            ...
            dic_n=10*dic_n, dic_dim=dic_dim,
            index=0,
        )
    ...
    def reparameterize(self):
        ...
        if isinstance(self.attn, vqAttention):
            print('vq attention reparameterize')
            self.attn.is_pre_cal = True
            self.attn.vq.is_pre_cal = True
    ...
    def forward(self, x):
        ...
        if not self.training and self.is_pre_cal:
            ...
            return x, loss_dict+atte_vq_loss, (feat0,feat)
