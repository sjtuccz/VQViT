...
Acknowledgments:
  尝试 Block中只使用一个残差结构，一个codebook可以跳过三层Linear。
"""
import logging
import math
...
class vqAttention(nn.Module):
    ...
    def reparameterize(self):
        ...
        '''
        print('using Attention proj reparameterize')
    ...
    def forward(self, x, shape=None):
        ...
        if self.is_pre_cal:
            ...
            return embedding_index, loss_dict
        else:
...
class vqBlock(nn.Module):
    ...
    def reparameterize(self):
        ...
        self.is_pre_cal = True
        self.pre_cal_dict = nn.Embedding(self.attn.vq.codebook_size, self.dim)
        vq_dict = self.attn.reparameterize()
        x = self.ls1(vq_dict)
        x = self.norm2(x)
        x = self.mlp(x)
        x = self.ls2(x)
        self.pre_cal_dict.weight.data.copy_(x)
        del self.ls1, self.norm2, self.mlp, self.ls2
    ...
    def forward(self, x):
        ...
        #     x = self.norm2(x)
        if self.is_pre_cal:
            embedding_index, vq_proj_loss = self.attn(self.norm1(x))
            x = self.pre_cal_dict(embedding_index)
        else:
        ...
        # return x, qkv_vq_loss, feat0
        return x, vq_proj_loss, feat
